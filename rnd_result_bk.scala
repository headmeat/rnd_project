...
val user_Result1 = user_Result_df_NaN.select("STD_NO", "SIMILARITY", "TRUST").rdd.collect.toList.map(_.toString)
...
case class myClass2(STD_NO: String, SIMILARITY: Double, TRUST: Double, Result: Double)

val list333 = user_Result1.map{ row =>
  val c1 = row._2 * a * w1
  val c2 = row._3 * b * w2
  val c3 = c1 + c2
  c3
...
val Result_stdno = list333.sortBy(x => x._4).reverse
Result_stdno.show()
