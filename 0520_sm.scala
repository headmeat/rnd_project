...
val base ="mongodb://rnd-cps01/cpmongo."
val output_base = "mongodb://rnd-cps01/cpmongo.USER_SIMILARITY"
val USER_LIST_FOR_SIM_output_base = "mongodb://rnd-cps01/cpmongo.USER_LIST_FOR_SIMILARITY"
val USER_SIM_output_base = "mongodb://rnd-cps01/cpmongo.USER_SIMILARITY"
val SREG_output_base = "mongodb://rnd-cps01/cpmongo.SREG_SIM"
val NCR_output_base = "mongodb://rnd-cps01/cpmongo.NCR_SIM"
val ACT_output_base = "mongodb://rnd-cps01/cpmongo.ACTIVITY_SIM"
//교과: SREG_SIM, 비교과: NCR_SIM, 자율활동: ACTIVITY_SIM
val Result_output_base = "mongodb://rnd-cps01/cpmongo.Recommend_Result"
...
//object Function{ 기존 옵젝 선언 위치
  ...
  val sbjt_res = sbjtFunc(spark, 20152611)
  val ncr_res = ncrFunc(spark, 20152611)
  val act_res = actFunc(spark, 20152611)
  val joinSim_res = joinSim(spark, 20152611)
  println("sbjtFunc : " + spark.time(sbjtFunc(spark, x)))
  println("ncrFunc : " + spark.time(ncrFunc(spark, 20152611)))
  println("actFunc : " + spark.time(actFunc(spark, 20152611)))
  println("sbjt_res : ", sbjt_res)
  println("ncr_res : ", ncr_res)
  println("act_res : ", act_res)

  //
  // stdarr.foreach(x =>{
  //
  //   //########################## 이렇게 stdNO 출력을 해줘야 코드가 안 멈춘다!!#############################
  //   println(x)
  //   val sbjt_res = sbjtFunc(spark, x)
  //   val ncr_res = ncrFunc(spark, x)
  //   val act_res = actFunc(spark, x)
  //   val joinSim_res = joinSim(spark, x)
  //   // println("sbjtFunc : " + spark.time(sbjtFunc(spark, x)))
  ...
  def calSim (spark:SparkSession, std_NO: Int) : DataFrame = {
    ...
    var stdNO_in_departNM_List = stdNO_in_departNM_sbjt.rdd.map(r=>r(0)).collect.toList.slice(0, 20).map(_.toString)
 ...
 // val star_subcd_avg_DF = star_subcd_joinDF.groupBy("NPI_AREA_SUB_CD").agg(avg("STAR_POINT"))
    ...
    stdNO_in_departNM_List.foreach{ stdNO =>
      ...
      val s = outAct_code_temp2.collect
...
val act_res = actFunc(spark, std_NO)
  ...
  def joinSim(spark:SparkSession, sbjt_res: DataFrame, ncr_res: DataFrame, act_res: DataFrame) : DataFrame = {
    ...
    MongoSpark.save(
      join_df.write
        .option("spark.mongodb.output.uri", "mongodb://rnd-cps01/cpmongo.USER_LIST_FOR_SIMILARITY")
        .mode("overwrite")
      )
...
def calculateSim(spark:SparkSession, std_NO: Int) : DataFrame = {
  ...
  if(userforSimilarity_df.count != 0){
    ...
    user_sim_join_df.write
        .option("spark.mongodb.output.uri", "mongodb://rnd-cps01/cpmongo.USER_SIMILARITY")
        .mode("overwrite")
      )
