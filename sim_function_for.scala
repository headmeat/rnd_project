...
var stdNO_star = cpsStarUri_sbjt_DF.select(cpsStarUri_sbjt_DF("STD_NO").as("STD_NO_2")).distinct
val joined_temp = stdNO_in_departNM_sbjt.join(stdNO_star, stdNO_in_departNM_sbjt("STD_NO") === stdNO_star("STD_NO_2"), "inner")
val joined = joined_temp.select(col("STD_NO"))
if(joined == null){
   println("null")
 }else{
   println("안ㅁㄹㅇㄴㄹ")
...
var sbjtCD_star_byStd_Map = stdNO_in_departNM_sbjt.foreach { row =>
  row.toSeq.foreach{stdNO =>
    // println(stdNO)
      var star_temp_bystdNO_DF = cpsStarUri_sbjt_DF.filter(cpsStarUri_sbjt_DF("STD_NO").equalTo(s"${stdNO}"))
      if (star_temp_bystdNO_DF == null){
...
stdNO_in_departNM_sbjt.foreach { row =>
  row.toSeq.foreach{stdNo =>
    ...
    var sbjtNM_by_stdNO = spark.createDataFrame(sc.emptyRDD[Row], schema_rdd)
    try{
      sbjtNM_by_stdNO = clPassUri_DF.filter(clPassUri_DF("STD_NO").equalTo(s"${stdNo}")).select(col("SBJT_KEY_CD"))
    }
    catch{
      case e: NoSuchElementException => println("ERROR!")
      val empty_df = sqlContext.createDataFrame(sc.emptyRDD[Row], schema_rdd)
