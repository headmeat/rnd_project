...
def mkDepGenComList (spark:SparkSession, stdNo : Int) : DataFrame = {
	...
	//std_no를 이용하여 getStdInfo 에서 일치하는 학생 정보(학번, 이름, 학과명, 성별, 졸업여부)만 추출 	//val stdInfo = getStdInfo.select("STD_NO", "KOR_NM", "SUST_CD_NM", "GEN_FG_NM", "GRADT_YN").filter("STD_NO" === stdNo)
...
var user_trust_df = user_trust.toDF("STD_NO", "ULT_SCORE") //최종 결과를 데이터프레임으로 만듬
...
//최종적으로 join해서 합치기
val final_rankList = Seq(NCR_std_Info_top5, CD01_top5, CD02_top5, CD03_avg)

var Result_All = PassUri_top5
final_rankList.foreach{ DF =>
